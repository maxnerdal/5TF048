...
namespace WebApp.Services
{
    ...
    public class BacktestingService : IBacktestingService
    {
        ...
        public BacktestingService(ApplicationDbContext context, ITradingBotService tradingBotService)
        {
            _context = context;
        ...
        public async Task<bool> ExecuteUserBotAsync(long userBotId)
        {
            try
            {
                ...
                if (!ShouldExecuteNow(userBot, config))
                {
        ...
        public async Task<bool> SimulateTradeAsync(long sessionId, string symbol, decimal currentPrice, DateTime timestamp)
        {
            try
            {
                ...
                var config = await _tradingBotService.GetBotConfigurationAsync(session.UserBotId);
                if (config == null)
                {
                    return false;
                }
        ...
        public async Task<long> RunHistoricalBacktestAsync(long userBotId, DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                var dcaFrequency = GetDCAFrequencyFromConfig(config);
        ...
        private bool ShouldExecuteNow(UserBot userBot, BotConfiguration config)
        {
        ...
        private async Task<bool> ExecuteTradeByStrategyAsync(long sessionId, string strategy, BotConfiguration config)
        {
            switch (strategy)
            {
        ...
        private async Task<bool> ExecuteDollarCostAverageAsync(long sessionId, BotConfiguration config)
        {
            try
            {
