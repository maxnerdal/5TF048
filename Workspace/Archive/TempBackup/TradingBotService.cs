...
namespace WebApp.Services
{
    ...
    public class TradingBotService : ITradingBotService
    {
        ...
        private readonly IBotStrategyFactory _strategyFactory;
        ...
        public async Task<bool> ExecuteBotStrategyAsync(long userBotId, DateTime? executionTime = null)
        {
            try
            {
                ...
                var executor = _strategyFactory.CreateExecutor(strategy);

                // Deserialize configuration based on strategy type
                var botConfig = _botConfigService.DeserializeConfiguration(config.Parameters, strategy);
                if (botConfig == null) return false;

                // Get recent market data for the target asset
                var targetAsset = GetTargetAsset(botConfig, strategy);
                ...
                var context = new BotExecutionContext
                {
                    ...
                    CurrentBalance = 1000m, // TODO: Calculate from trades
                ...
                if (result.Success)
                {
                    // Execute any trade actions
                    foreach (var tradeAction in result.TradeActions)
                    {
                    ...
                    {
                        await UpdatePerformanceFromMetricsAsync(sessionId, result.Metrics);
                    }
