...
{
    /// <summary>
    /// Service for managing trading bots, user bots, trading sessions, and related operations
    ...
    public class TradingBotService : ITradingBotService
    {
        ...
        #region Universal Bot Execution

        /// <summary>
        /// Execute any bot strategy using the unified strategy pattern
        /// This method works for DCA, Grid, Momentum, or any other bot type
        /// </summary>
        public async Task<bool> ExecuteBotStrategyAsync(long userBotId, DateTime? executionTime = null)
        {
            try
            {
                ...
                // For now, we'll handle DCA strategy directly without strategy factory
                // TODO: Re-implement strategy factory pattern when needed
                if (strategy != "DCA")
                {
                    return false; // Only DCA supported for now
                }

                // Deserialize DCA configuration from JSON
                ...
                // Get recent market data for the target asset  
                var targetAsset = botConfig.TargetAsset; // DCA config has TargetAsset property
                var marketData = await GetRecentMarketDataAsync(targetAsset);

                // Get existing trades for this bot
                ...
                if (sessionId == 0)
                {
                    ...
                    sessionId = await CreateTradingSessionAsync(userBotId, "Live", 1000m);
                }

                var existingTrades = await GetTradesAsync(sessionId);

                // Execute DCA strategy directly (simplified approach)
                // TODO: Restore strategy factory pattern when needed
        ...
        /// <summary>
        /// Get target asset symbol from configuration based on strategy
        /// </summary>
        private string GetTargetAsset(BaseBotConfiguration config, string strategy)
        {
            return strategy.ToUpper() switch
            {
                "DCA" => ((DcaBotConfiguration)config).TargetAsset,
                // Add other strategy asset extraction here
                _ => "BTC" // Default fallback
            };
        }

        /// <summary>
        /// Get recent market data for backtesting and execution
        /// </summary>
        private async Task<List<MarketData>> GetRecentMarketDataAsync(string symbol, int count = 100)
        {
        ...
        /// <summary>
        /// Update performance metrics from execution result
        /// </summary>
        private async Task<bool> UpdatePerformanceFromMetricsAsync(long sessionId, Dictionary<string, object> metrics)
        {
            try
            {
                var totalInvested = Convert.ToDecimal(metrics.GetValueOrDefault("TotalInvested", 0m));
                var totalValue = Convert.ToDecimal(metrics.GetValueOrDefault("TotalValue", totalInvested));
                var totalTrades = Convert.ToInt32(metrics.GetValueOrDefault("TotalTrades", 0));
                var winRate = metrics.ContainsKey("WinRate") ? Convert.ToDecimal(metrics["WinRate"]) : (decimal?)null;

                return await UpdatePerformanceMetricsAsync(sessionId, totalInvested, totalValue, totalTrades, winRate);
            }
            catch
            {
                return false;
        ...
        /// <summary>
        /// Execute DCA strategy directly (simplified approach without strategy factory)
        /// </summary>
        private async Task<bool> ExecuteDcaStrategyAsync(long userBotId, long sessionId, DcaBotConfiguration config, List<MarketData> marketData, List<Trade> existingTrades, DateTime executionTime)
        {
            try
            {
                // Get current price for the target asset
                ...
                var fee = tradeAmount * 0.001m; // 0.1% fee

                // Create the trade
                var success = await CreateTradeAsync(
                    sessionId,
                    config.TargetAsset,
                    "BUY",
                    latestPrice,
